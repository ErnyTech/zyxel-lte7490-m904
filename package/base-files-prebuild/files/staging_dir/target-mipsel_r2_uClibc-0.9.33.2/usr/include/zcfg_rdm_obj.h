...
typedef struct rdm_FeatureFlag_s {   // RDM_OID_FEATURE_FLAG 
	...
	char UserAgent[33];
	uint32_t SupportedFactoryResetUnpurgedList;
...
typedef struct rdm_L2TPv3_s {   // RDM_OID_L2_T_PV3 
	bool Enable;
	char LocalIPAddress[65];
	char LocalTunnelAddress[65];
	char RemoteIPAddress[65];
	char RemoteTunnelAddress[65];
	uint32_t LocalTunnelID;
	uint32_t RemoteTunnelID;
