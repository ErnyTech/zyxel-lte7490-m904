...
rm -f "$prefix/bin/busybox_zy" || exit 1
mkdir -p "$prefix/bin" || exit 1
install -m 755 busybox_zy "$prefix/bin/busybox_zy" || exit 1
...
for i in $h; do
	...
	if [ "$scriptwrapper" = "y" ]; then
		if [ "$swrapall" != "y" ] && [ "$i" = "/bin/sh" ]; then
			ln $linkopts busybox_zy "$prefix/$i" || exit 1
		else
			rm -f "$prefix/$i"
			echo "#!/bin/busybox_zy" >"$prefix/$i"
			chmod +x "$prefix/$i"
		fi
		echo "	$prefix/$i"
	else
		if [ "$2" = "--hardlinks" ]; then
			bb_path="$prefix/bin/busybox_zy"
		else
			case "$appdir" in
			/)
				bb_path="bin/busybox_zy"
			;;
			/bin)
				bb_path="busybox_zy"
			;;
