...
void sendDigestChallenge(CgHttpRequest *httpReq, char *realm)
{
	...
	//printf("%s() Jessie newEntry->realm=%s, realm=%s\n", __FUNCTION__, newEntry->realm, realm);//Jessie
	newEntry->opaque = CG_HTTP_DEFAULT_OPAQUE;
        //printf("%s() Jessie newEntry->opaque=%s\n", __FUNCTION__, newEntry->opaque);//Jessie
	newEntry->qop = CG_HTTP_DEFAULT_QOP;
        //printf("%s() Jessie newEntry->qop=%s\n", __FUNCTION__, newEntry->qop);//Jessie
	newEntry->algorithm = CG_HTTP_DEFAULT_ALGORITHM;

	strcpy(newEntry->clientHost, cg_http_request_getremoteaddress(httpReq));
//	printf("%s() Jessie newEntry->clientHost=%s\n", __FUNCTION__, newEntry->clientHost);//Jessie
...
//		printf("%s() Jessie 333333333333333\n", __FUNCTION__);//Jessie
...
//	printf("%s() Jessie authMethod=%s\n", __FUNCTION__, authMethod);//Jessie
...
//		printf("%s() Jessie auth->nonce=%s\n", __FUNCTION__, auth->nonce);//Jessie
		...
		//printf("%s() Jessie auth->response=%s\n", __FUNCTION__, auth->response);//Jessie
...
//		printf("%s() Jessie auth->username=%s, user=%s\n", __FUNCTION__, auth->username, user);//Jessie
		...
		now = digestHead;
		while(now != NULL){
//			printf("%s() Jessie now->clientHost=%s,auth->clientHost=%s\n", __FUNCTION__, now->clientHost,auth->clientHost);//Jessie
			if(!strcmp(now->clientHost, auth->clientHost)){
				found = TRUE;
//				printf("%s() Jessie found!!!!\n", __FUNCTION__);//Jessie
...
//		printf("%s() Jessie found=%d\n", __FUNCTION__, found);//Jessie
		if(!found){
#ifdef JESSIE_DBG
...
//			printf("%s() Jessie now->qop\n", __FUNCTION__);//Jessie
			...
			if(!tmp){
				...
				auth->qop		= tmp;
//				printf("%s() Jessie auth->qop=%s\n", __FUNCTION__, auth->qop);//Jessie
...
//			printf("%s() Jessie now->algorithm\n", __FUNCTION__);//Jessie
			...
			if(!tmp){
				...
				auth->algorithm	= tmp;
//				printf("%s() Jessie auth->algorithm=%s\n", __FUNCTION__, auth->algorithm);//Jessie
...
//			printf("%s() Jessie now->opaque\n", __FUNCTION__);//Jessie
		...
		if(!tmp)
			...
			auth->cnonce	= tmp;
//		printf("%s() Jessie get auth->cnonce from tmp\n", __FUNCTION__);//Jessie
		tmp 				= cg_http_request_getnc(httpReq);
		if(!tmp)
			auth->nc		= 0;
		else
			sscanf(tmp, "%08x", &auth->nc);
//		printf("%s() Jessie get nc from sscanf tmp\n", __FUNCTION__);//Jessie
		method				= cg_http_request_getmethod(httpReq);
		uripath				= cg_http_request_geturi(httpReq);
...
*/
		memset(md5Input, "\0", sizeof(md5Input));
		snprintf(md5Input, sizeof(md5Input), "%s:%s:%s", user, auth->realm, passwd);
//		printf("%s() Jessie md5Input=%s to get ha1\n", __FUNCTION__, md5Input);//Jessie
		cg_str2md5(md5Input,ha1);
		memset(md5Input, "\0", sizeof(md5Input));
		snprintf(md5Input, sizeof(md5Input), "%s:%s", method, uripath);
//		printf("%s() Jessie222 md5Input=%s to get ha2\n", __FUNCTION__, md5Input);//Jessie
		cg_str2md5(md5Input,ha2);
...
//			printf("%s() Jessie 11111111\n", __FUNCTION__);//Jessie
			...
			snprintf(md5Input, sizeof(md5Input), "%s:%s:%s", ha1, auth->nonce, ha2);
//			printf("%s() Jessie333333 md5Input=%s\n", __FUNCTION__, md5Input);//Jessie
			cg_str2md5(md5Input,digest);
		}
		else{
//			printf("%s() Jessie 444444444444\n", __FUNCTION__);//Jessie
			if((auth->algorithm)&&(strcmp(auth->algorithm, "MD5") != 0)&&(strcmp(auth->algorithm,"MD5-sess") != 0)){
#ifdef JESSIE_DBG
...
//			printf("%s() Jessie6666 auth->qop=%s\n", __FUNCTION__, auth->qop);//Jessie
			if(strstr(auth->qop, "auth-int") != NULL){
				entityBody = cg_http_request_getcontent(httpReq);
				cg_str2md5(entityBody, hEntity);
				snprintf(md5Input, sizeof(md5Input), "%s:%s:%s", method, uripath, hEntity);
//				printf("%s() Jessie7777 md5Input=%s\n", __FUNCTION__, md5Input);//Jessie
				cg_str2md5(md5Input, ha2);
			}

			if((auth->algorithm) && (strcmp(auth->algorithm, "MD5-sess") == 0)){
//				printf("%s() Jessie8888888\n", __FUNCTION__);//Jessie
				snprintf(md5Input, sizeof(md5Input), "%s:%s:%s", ha1, auth->nonce, auth->cnonce);
				cg_str2md5(md5Input, ha1);
			}
			...
			snprintf(md5Input, sizeof(md5Input), "%s:%s:%08x:%s:%s:%s",
											...
											auth->qop,
											ha2);
//			printf("%s() Jessie9999 md5Input=%s\n", __FUNCTION__, md5Input);//Jessie
			cg_str2md5(md5Input,digest);
		}
//		printf("%s() JessieAAA digest=%s,auth->response=%s\n", __FUNCTION__, digest, auth->response);//Jessie
