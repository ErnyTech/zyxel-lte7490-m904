...
extern int checkCertificate;
extern int checkCertificateCN;

#define kRootPath           "/"
/*
    Table for converting scheme strings to scheme enums and default ports.
*/
#define kSecure TRUE
						...
						const char * availableMode) 
{
	...
	char tmpUrlPtr[STRSIZE(SIZE_URL)]={0};
	...
	int lenUrl = 0;
	memcpy(tmpUrlPtr, theUrl, ((lenUrl = strlen(theUrl))>SIZE_URL) ? SIZE_URL : lenUrl);
	memset( theHostipPtr, 0, sizeof(theHostipPtr));
	*theHostName = '\0';
	*thePort = kDefaultHttpPort;
	//*thePath = '\0';
	memset(thePath, 0, SIZE_URL);
	*hostName = '\0';
	/*
	...
	*/
	...
	if( theHostPtr != NULL){
		...
		if( thePathPtr != NULL){
			...
			snprintf(thePath, SIZE_URL, "/%s", thePathPtr);
		}
		else {
	...
	if(strstr(theHostPtr, "[") != NULL){
		...
		*/
		if ( isValidIP(theHostName) ){
			...
			{
				USP_LOG_Info("%s:  olveSrvUrlName error! url=%s\n", __FUNCTION__, theHostName);
...
void cwmp_http_request_set_cookieset( CgHttpRequest *httpReq, cookieInfo_t *cookieinfo ){
        ...
        USP_LOG_Info(  "httpCookieCount %d", cookieinfo->httpCookieCount);
...
bool cwmp_http_response_get_cookieset( CgHttpResponse *httpRes, cookieInfo_t *cookieinfo ){
        ...
        while((header = cg_http_headerlist_get(header, CG_HTTP_SETCOOKIE2))) {
                int cookieNameLen = 0;
                USP_LOG_Info(  "httpRes has SETCOOKIE2");
                cookie = NULL;
                if((cookie = cg_http_header_getvalue(header)) && (index < MAX_NUM_COOKIES)) {
                        USP_LOG_Info(  "cookie data: %s", cookie);
        ...
        while((header = cg_http_headerlist_get(header, CG_HTTP_SETCOOKIE))) {
                int cookieNameLen = 0;
                USP_LOG_Info(  "httpRes has SETCOOKIE");
                cookie = NULL;
                if((cookie = cg_http_header_getvalue(header)) && (index < MAX_NUM_COOKIES)) {
                        USP_LOG_Info(  "cookie data: %s", cookie);
                        cg_http_cookie_header_parse(cookie, &cookieinfo->cookiedata[index]);
...
CgHttpResponse *cwmp_http_request_post(CgHttpRequest *httpReq, char *hostName, char *ipaddr, int port, bool isSecure, char *savedFile)
{
        ...
        if( httpReq->sock == NULL ){
                ...
                if (tr069_cg_socket_connect_MARK(sock, ipaddr, port, boundIfName, MARK) == FALSE) {
                        ...
                        USP_LOG_Info(  "cg_socket_connect fail, ipaddr=%s, port=%d", ipaddr, port);
        ...
        if (method == NULL || uri == NULL || version == NULL) {
                ...
                httpReq->sock = NULL;
                USP_LOG_Info(  "method == NULL || uri == NULL || version == NULL");
                return httpReq->httpRes;
        }
        ...
        if(cg_http_response_read(httpReq->httpRes, sock, cg_http_request_isheadrequest(httpReq), savedFile) <= 0){
                USP_LOG_Info(  "cg_http_response_read fail");
                cwmp_http_msgauthdata_delete(httpReq);
                cwmp_http_sock_close(sock);
                httpReq->sock = NULL;
        ...
        */
        if((cg_strcaseeq(cg_http_packet_getconnection((CgHttpPacket*)httpReq->httpRes), CG_HTTP_CLOSE) == TRUE)){
                USP_LOG_Info(  "packet CG_HTTP_CLOSE");
                cwmp_http_msgauthdata_delete(httpReq);
                cwmp_http_sock_close(sock);
                httpReq->sock = NULL;
        }
...
CgHttpResponse *cwmp_http_request_post_with_auth(CgHttpRequest *httpReq, char *hostName, char *ipaddr, int port, char *user, char *passwd, bool isSecure, char *savedFile)
{
        ...
        if((hasCookie = cwmp_http_response_get_cookieset(httpRes, &cookieinfo))){
                ...
                USP_LOG_Info( "http auth message has cookie");
        ...
        if ((strcmp(authHeader, "Basic") == 0) || (strcmp(authHeader, "basic") == 0))  /*basic authentication*/
        {
                ...
                httpRes = cwmp_http_request_post(httpReq, hostName, ipaddr, port, isSecure, savedFile);
                if(httpRes == NULL)
                        USP_LOG_Info(  "http response fail");
...
* Output        :
*************************/
soapResponse_t *cwmp_soapreq_post( soapRequest_t *soapReq, servInfo_t *servInfo, char   *savedFile){
        ...
        USP_LOG_Info(  "http response status code=%d", cg_http_response_getstatuscode(httpRes));
        ...
        {
                do{
                        USP_LOG_Info(  "CG_HTTP_STATUS_FOUND or CG_HTTP_STATUS_TEMPORARY_REDIRECT");
                        redirectUrl = cg_http_packet_getheadervalue((CgHttpPacket*)httpRes,CG_HTTP_LOCATION);
                        USP_LOG_Info(  "close the current socket, redirectUrl=%s", redirectUrl);
                        ...
                        if (cwmp_parseurl( redirectUrl, thehostname, (struct sockaddr *)&(redirectServInfo.ipAddress), &redirectServInfo.port, redirectServInfo.path, &redirectServInfo.isSecure, redirectServInfo.hostname, eMULTIP_CHOOSE_REGULAR, "DualStack"/*, gTR369CB.acsInfo.X_ZYXEL_CurrentMode*/ ) == USP_ERR_OK) {
...
soapRequest_t *cwmp_upload_build( void *sessCb, servInfo_t *serInfo){
	...
	USP_LOG_Info("rpcUpload->fileType=%s\n", rpcUpload->fileType);
	transferInfo->localFilePath[0] = '\0';

	if( !strcmp( rpcUpload->fileType, UPLOAD_TYPE_CFG) ){
		transferInfo->uploadType = eUploadTypeCfg;
		strcpy(transferInfo->localFilePath, LOCAL_CONFIG_PATH);
	}
	else if(strstr( rpcUpload->fileType, UPLOAD_TYPE_LOG)){
		transferInfo->uploadType = eUploadTypeLog;

		if(!strcmp( rpcUpload->fileType, UPLOAD_TYPE_LOG) ){
			strcpy(transferInfo->localFilePath, LOCAL_LOG_PATH);
		}
		else{
			objIndex_t objIid;
			rdm_DevInfoVendorLogFile_t *vendorLogObj = NULL;
			IID_INIT(objIid);
			objIid.level = 1;
			if(sscanf(rpcUpload->fileType, UPLOAD_TYPE_LOG" %hhu", &objIid.idx[0]) == 1){
				if(zcfgFeObjStructGetWithoutUpdate(RDM_OID_DEV_INFO_VENDOR_LOG_FILE, &objIid, (void **)&vendorLogObj) == ZCFG_SUCCESS){
					if(strlen(vendorLogObj->Name)){
						#ifdef ZYXEL_DEVICEINFO_VENDOR_LOG_SAVE_TO_FLASH
						if(objIid.idx[0] < 4){
						#endif
						sprintf(transferInfo->localFilePath, "%s%s", LOCAL_LOG_FOLDER_PATH, vendorLogObj->Name);
						#ifdef ZYXEL_DEVICEINFO_VENDOR_LOG_SAVE_TO_FLASH
						} else {
							sprintf(transferInfo->localFilePath, "%s%s", FLASH_LOG_FOLDER_PATH, vendorLogObj->Name);
						}
						#endif
					}
					zcfgFeObjStructFree(vendorLogObj);
				}
			}
		}
	}

	if(transferInfo->localFilePath[0] == '\0'){
		USP_LOG_Info(  "not support upload type %s", rpcUpload->fileType);
		return NULL;
	}

	soapReq = cwmp_soapreq_new();
	if( soapReq == NULL ){
		USP_LOG_Info(  "create soap request fail!");
		return NULL;
	}

	/*set content*/
	fp = fopen( transferInfo->localFilePath, "r" );
	if (!fp) {
		USP_LOG_Info(  "fopen %s fail", transferInfo->localFilePath );
		cwmp_soapreq_delete( soapReq );
		soapReq = NULL;
		goto END;
	}
	...
	rret = fread(content, contentLen, 1, fp);
	fclose(fp);

	if( 1 != rret ){
		USP_LOG_Info(  "fread  %s fail", transferInfo->localFilePath);
		cwmp_soapreq_delete( soapReq );
		soapReq = NULL;
		goto END;
	}
	...
	if (!(TR369_FEATURE_Flag & TR369_FEATURE_UPLOAD_BY_PUT)) {
...
int cwmp_upload_parse( soapResponse_t *soapResp , soapRequest_t *soapReq, transferInfo_t *transferInfo){
        ...
        if(cg_http_response_getstatuscode(theHttpRes)!=0 && cg_http_response_issuccessful(theHttpRes) == 0 &&
                ...
                USP_LOG_Info(  "http response not success [status code=%d]", cg_http_response_getstatuscode(theHttpRes));
                if(cg_http_response_authfail(theHttpRes)){
                        USP_LOG_Info( "TR369_FILE_TRANSFER_SERVER_AUTHENTICATION_FAILURE");
                }else
						USP_LOG_Info( "TR369_UPLOAD_FAILURE");
                ret = USP_ERR_INTERNAL_ERROR;
                goto END;
        }
        ...
        if ( cg_http_response_getstatuscode(theHttpRes) != CG_HTTP_STATUS_OK){
                USP_LOG_Info(  "http response not success (200)");
                USP_LOG_Info( "TR369_UPLOAD_FAILURE");
                ret = USP_ERR_INTERNAL_ERROR;
                goto END;
        }
...
* Output        :
*************************/
int cwmp_send_request_downup( tr369ConnCB_t *connCbPtr ){
        ...
        if( connCbPtr == NULL ){
                USP_LOG_Info(  "connCbPtr == NULL");
                return USP_ERR_INTERNAL_ERROR;
        }
        theSessCb = connCbPtr->sessionCb;
        if( theSessCb == NULL ){
                USP_LOG_Info(  "theSessCb == NULL");
                return USP_ERR_INTERNAL_ERROR;
        }
        ...
        /*assign transferinfo */
        transferInfo = theSessCb->transferInfo;
        if(transferInfo == NULL) return USP_ERR_INTERNAL_ERROR;
        ...
        if(transferInfo->rpcDownload){
			theSessCb->soapResp = cwmp_soapreq_post(theSoapReq, &connCbPtr->servInfo, transferInfo->localFilePath);
        }
        else
                theSessCb->soapResp = cwmp_soapreq_post(theSoapReq, &connCbPtr->servInfo, NULL);
...
* Output 	:
*************************/
int cwmp_sess_file_create_upload( tr369ConnCB_t *connCbPtr ){
	...
	if( theSessCb->soapReq == NULL ){
		USP_LOG_Info(  "send request fail!");
		return USP_ERR_INTERNAL_ERROR;
	}

	if( cwmp_send_request_downup( connCbPtr )!= USP_ERR_OK ){
		USP_LOG_Info(  "send request fail!");
		return USP_ERR_INTERNAL_ERROR;
	}

	if( cwmp_upload_parse( theSessCb->soapResp, theSessCb->soapReq, theSessCb->transferInfo) != USP_ERR_OK){
		USP_LOG_Info(  "Upload parse fail!");
		return USP_ERR_INTERNAL_ERROR;
	}
...
void cwmp_conn_remove( tr369ConnCB_t *connCbPtr ){
		...
		if( connCbPtr == NULL ){
			USP_LOG_Error("%s connCbPtr == NULL!\n", __FUNCTION__);
			return;
        }
...
* Output 	:
*************************/
int cwmp_sess_file_proc( tr369ConnCB_t *theConnCb ){
	if(theConnCb){
		if( cwmp_sess_file_create_upload(theConnCb) == USP_ERR_OK ){
			USP_LOG_Info("cwmp_sess_file_create_upload OK\n");
		} else {
			USP_LOG_Error("cwmp_sess_file_create_upload Fail\n");
			return USP_ERR_COMMAND_FAILURE;
		}
	} else {
		USP_LOG_Error("%s theConnCb == NULL!\n", __FUNCTION__);
		return USP_ERR_COMMAND_FAILURE;
	}
...
#define UUID_LEN 36
int zyUtilityGenerateUuid(char *uuid_string)
{
#if 1
	uuid_t uuid;
	char uuidStr[256] ={0};
	...
	snprintf(uuid_string, (UUID_LEN + 1), "%s", uuidStr);
	printf("[Marcus 20220804] %s:%d, uuid %s.\n", __func__, __LINE__, uuid_string);
#else
	...
	memcpy(mac,(const void *)ether_aton(macaddr),ETHER_ADDR_LEN);
	/* Generate hash */
#ifdef USE_OPENSSL_MD5
	MD5_Init(&ctx);
	MD5_Update(&ctx, mac, sizeof(mac));
	MD5_Update(&ctx, deviceType, strlen(deviceType));
	MD5_Final(digest, &ctx);
#else
	MD5Init(&ctx);
	MD5Update(&ctx, mac, sizeof(mac));
	MD5Update(&ctx, deviceType, strlen(deviceType));
	MD5Final(digest, &ctx);
#endif

	sprintf((char*)uuid_string, "%02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x%02x%02x%02x%02x",
