...
bool importCertificate(char *name, char *filePath)
{
	...
	snprintf(path, sizeof(path), "Device.LocalAgent.Certificate.%d", objIid.idx[0]);
	printf("path = %s\n", path);
...
int localAgentAddCertificate(oper_input_cond_t *cond)
{
	...
	fwrite(certificate, 1, strlen(certificate), fpCert);
	fwrite("\n-----END CERTIFICATE-----\n", 1, strlen("\n-----END CERTIFICATE-----\n"), fpCert);
...
int localAgentAddMyCertificate(oper_input_cond_t *cond)
{
	...
	while(zcfgFeObjJsonGetNext(RDM_OID_DEV_SEC_CERT, &certObjIid, &certObj) == ZCFG_SUCCESS) {
		if(strcmp(json_object_get_string(json_object_object_get(certObj, "X_ZYXEL_Name")), alias) == 0){ //Comparison input args & database X_ZYXEL_Name is identical
			...
			if(zcfgFeObjJsonGet(RDM_OID_DEV_LOCAL_AGENT_CERTIFICATE, &certObjIid, &localAgentCertObj) != ZCFG_SUCCESS) {
				USP_LOG_Error("%s(%d) Error: Get RDM_OID_DEV_LOCAL_AGENT_CERTIFICATE Fail.\n", __func__, __LINE__);
				return USP_ERR_INTERNAL_ERROR;
			}
			...
			if(strcmp(json_object_get_string(json_object_object_get(localAgentCertObj, "Alias")), alias) == 0 \
				...
				snprintf(path, sizeof(path), "Device.LocalAgent.Certificate.%d", certObjIid.idx[0]);
...
int localAgentCertificateDelete(oper_input_cond_t *cond)
{
	...
	char *strtmp;
	int len = strlen("Device.LocalAgent.Certificate.");
	
	USP_LOG_Info("Now enter %s(%d).\n", __func__, __LINE__);
	strtmp = cond->parentPath + len;
	len = strlen(strtmp);
	strtmp[len - 1] = '\0';
	snprintf(securityCertificatePath, sizeof(securityCertificatePath), "Device.Security.Certificate.%s", strtmp);
