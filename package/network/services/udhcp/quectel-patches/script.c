			...
			if(type_p->code == DHCP_SUBNET)
			{
                ...
                if(ret != ZCFG_SUCCESS)
                {
                    ...
                    if (prefix == 0) {
                        ...
                        // system("echo 0 > /proc/sys/net/ipv4/conf/all/proxy_arp");
                    } else {

                        // __YuChih__, TBD: proxy arp should be enabled by another config.
                        system("echo 1 > /proc/sys/net/ipv4/conf/all/proxy_arp");

                        // __YuChih__, 20210408: proxy arp enabled cause gratuitous arp with IP duplicated detected.
                        // set arp_announce to 1.
                        system("echo 1 > /proc/sys/net/ipv4/conf/all/arp_announce");
