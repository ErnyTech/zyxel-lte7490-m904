                                     ...
                                     bool isBootstrap)
{
    ...
    if (NULL != securityObj)
    {
        ...
        // Manually create an hardcoded instance => Jessie read from zcfg
        targetP = (security_instance_t *)lwm2m_malloc(sizeof(security_instance_t));
        if (NULL == targetP)
        {
        ...
        memset(targetP, 0, sizeof(security_instance_t));
        targetP->instanceId = instanceCount;//Jessie
		instanceCount++;//Jessie
        targetP->uri = (char*)lwm2m_malloc(strlen(serverUri)+1);
		if(targetP->uri){
			memset(targetP->uri, 0, strlen(serverUri)+1);
			strcpy(targetP->uri, serverUri);//Please make sure targetP->uri contains two parts: url and port number.
		} else {
            lwm2m_free(securityObj);
            return NULL;
        }

        targetP->securityMode = LWM2M_SECURITY_MODE_NONE;//Jessie??
                      ...
                      uint16_t secObjInstID)
{
    ...
    if (NULL != targetP)
    {
		//printf("%s() Jessie NULL != targetP   uri=%s\n", __FUNCTION__, targetP->uri);
