                               ...
                               coap_packet_t * message)
{
    if (COAP_204_CHANGED == message->code)
    {
        ...
        zcfgLogPrefix(ZCFG_LOG_INFO, ZCFG_LOGPRE_LWM2M, "Received ACK/2.04, Bootstrap pending, waiting for DEL/PUT from BS server...");
...
#warning No supported content type for bootstrap. Bootstrap will always fail.
...
#endif
#ifdef HTTP_PROTOCOL
        bootstrapServer->sessionH = lwm2m_connect_server(bootstrapServer->secObjInstID, context->userData, context);
#endif
    }
    if (bootstrapServer->sessionH != NULL)
    {
        ...
        zcfgLogPrefix(ZCFG_LOG_INFO, ZCFG_LOGPRE_LWM2M, "Bootstrap server connection opened");
        ...
        if (transaction_send(context, transaction) == 0)
        {
            ...
            zcfgLogPrefix(ZCFG_LOG_INFO, ZCFG_LOGPRE_LWM2M, "CI bootstrap requested to BS server");
    ...
    else
    {
        ...
        zcfgLogPrefix(ZCFG_LOG_ERR, ZCFG_LOGPRE_LWM2M, "Connecting bootstrap server failed");
                    ...
                    time_t * timeoutP)
{
    ...
    while (targetP != NULL)
    {
		...
		if(targetP->status == STATE_BS_FAILING || targetP->status == STATE_BS_FAILED){
			zcfgLogPrefix(ZCFG_LOG_ERR, ZCFG_LOGPRE_LWM2M, "Final status: %s", STR_STATUS(targetP->status));
		}
		else{
			zcfgLogPrefix(ZCFG_LOG_INFO, ZCFG_LOGPRE_LWM2M, "Final status: %s", STR_STATUS(targetP->status));
		}
                               ...
                               void * fromSessionH)
{
     ...
     && bootstrapServer->status == STATE_BS_PENDING)
    {
        if (object_getServers(context, true) == 0)
        {
            ...
            zcfgLogPrefix(ZCFG_LOG_INFO, ZCFG_LOGPRE_LWM2M, "Bootstrap server status changed to STATE_BS_FINISHING");
...
#endif
#ifdef HTTP_PROTOCOL
            targetP->sessionH = lwm2m_connect_server(targetP->secObjInstID, contextP->userData, contextP);
#endif
        }
        targetP = targetP->next;
    }
...
static uint8_t prv_checkServerStatus(lwm2m_server_t * serverP)
{
    ...
    default:
        ...
        zcfgLogPrefix(ZCFG_LOG_INFO, ZCFG_LOGPRE_LWM2M, "Returning COAP_IGNORE");
        return COAP_IGNORE;
    }
                                ...
                                coap_packet_t * response)
{
    ...
    zcfgLogPrefix(ZCFG_LOG_INFO, ZCFG_LOGPRE_LWM2M, "[%s] Code: %02X",__func__, message->code);
    LOG_URI(uriP);
    ...
    zcfgLogPrefix(ZCFG_LOG_INFO, ZCFG_LOGPRE_LWM2M, "Server status: %s", STR_STATUS(serverP->status));
