...
static bool checkConnStatValueChange(int resourceId, int resourceInsId/*multi resource using, single resource no effect*/, conn_stat_instance_t * targetP, char * zcfgParamValue)
{
        ...
        else
        {
			...
			case RES_O_TX_DATA:
				...
				if(ret == ZCFG_SUCCESS){
					...
					if(checkConnStatValueChange((*dataArrayP)[i].id, -1, targetP, desValue))
					{
