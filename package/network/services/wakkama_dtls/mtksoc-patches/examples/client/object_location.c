...
static bool checkLocationValueChange(int resourceId, int resourceInsId/*multi resource using, single resource no effect*/, location_instance_t * targetP, char * zcfgParamValue)
{
...
static uint8_t prv_location_discover(lwm2m_context_t *contextP,
                                   uint16_t instanceId,
                                   int * numDataP,
...
{   
    ...
    do
    {
		...
		case RES_O_ALTITUDE:
            ...
            if(checkLocationValueChange((*tlvArrayP)[i].id, -1, targetP, desValue))
            {
                setChangeValueList(objectP->objID, -1,  (*tlvArrayP)[i].id,  -1);
            }

			if((*tlvArrayP)[i].id == RES_O_ALTITUDE){
				//The data stored in data model is INT type multipied 10
				lwm2m_data_encode_float((atof(desValue)/10.0), *tlvArrayP + i);
			}
			else{
				//The data stored in data model is INT type multipied 1000000
				lwm2m_data_encode_float((atof(desValue)/1000000.0), *tlvArrayP + i);
			}
            result = COAP_205_CONTENT;
            break;
		case RES_M_TIMESTAMP:
