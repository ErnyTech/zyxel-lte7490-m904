...
static bool checkServerValueChange(int resourceId, int resourceInsId/*multi resource using, single resource no effect*/, server_instance_t * targetP, char * zcfgParamValue)
{
...
uint8_t prv_server_delete(lwm2m_context_t *contextP,
                                 uint16_t id,
...
{
    server_instance_t * targetP;
	#if 1//LWM2M_SERVER_BOOTSTRAP_ID
    ...
    case LWM2M_SERVER_UPDATE_ID:
        // executed in core, if COAP_204_CHANGED is returned
		resultUpdate = lwm2m_update_registration(contextP, targetP->shortServerId, true);//if targetP->shortServerId is replaced by 0, lwm2mclient will send registration update to all connected servers
		if (resultUpdate != 0)
		{
	...
	case LWM2M_SERVER_BOOTSTRAP_ID:
		...
		serverP = contextP->bootstrapServerList;
		if (serverP != NULL)//while??
		{
			bootstrap_start(contextP);
			contextP->state = STATE_BOOTSTRAPPING;
			bootstrap_step(contextP, tv_sec, &(tv.tv_sec));
...
#endif								  
{
	...
	#endif
    objectP->instanceList = lwm2m_list_remove(objectP->instanceList, instanceId, &serInstance);
    if (NULL == serInstance) return COAP_404_NOT_FOUND;
	#if 1//Jessie problem222
	printf("%s() Jessie serInstance found != NULL\n", __FUNCTION__);
	#endif
    lwm2m_free(serInstance);
...
{
	if(object)//Jessie problem222
    fprintf(stdout, "  /%u: Server object, instances:\r\n", object->objID);
	else//Jessie problem222
		return;//Jessie problem222
                                   ...
                                   bool storing)
{
    ...
    if (NULL != serverObj)
    {
        ...
        while(zcfgFeObjStructGetNext(RDM_OID_LWM2M_SERVER, &objIid, (void **)&lwm2mServerObj) == ZCFG_SUCCESS)
        {
            ...
            memset(serverInstance, 0, sizeof(server_instance_t));
            serverInstance->instanceId = objIid.idx[0] - 1;//Jessie
            ...
            snprintf(serverInstance->binding, sizeof(serverInstance->binding), "%s", lwm2mServerObj->Binding_Preference_7);
