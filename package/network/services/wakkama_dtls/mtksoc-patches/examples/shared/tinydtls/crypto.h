...
//This "define" comes from the reason that the PSK key length from Vodafone is 24 Bytes.
//Allow the PSK input key maximum length to 32 bytes.
#define DTLS_MAX_KEY_LENGTH    32
...
#else
//For fixing the maximum length issue from Vodafone
#define MAX_KEYBLOCK_LENGTH  \
  (2 * DTLS_MAC_KEY_LENGTH + 2 * DTLS_MAX_KEY_LENGTH  + 2 * DTLS_IV_LENGTH)
#endif
...
#else
#define DTLS_PSK_MAX_KEY_LEN DTLS_MAX_KEY_LENGTH
#endif
