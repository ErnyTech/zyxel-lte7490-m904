...
dal_param_t NETWORK_MAP_param[] = 
{
	...
	{"X_ZYXEL_PhysAddressOUI",			dalType_string,		0,	0,	NULL},
...
//MTKSOC Patches
					...
					IID_INIT(iid);
					iid.level = 1;
					sscanf(LowerLayers, "Cellular.Interface.%hhu", &(iid.idx[0]));
					#endif
...
//MTKSOC Patches
	...
	if((ret = zcfgFeObjJsonGet(RDM_OID_CELL_INTF_USIM, &iid, &cellIntfSimObj)) != ZCFG_SUCCESS)
        ...
        //Get LowerLayers of Device.Ethernet.Link.2
        if((ret = zcfgFeObjJsonGet(RDM_OID_ETH_LINK, &iid, &ethLinkObj)) != ZCFG_SUCCESS)
                goto zcfgFeDalCardPageStatusGetFail;
        sscanf(json_object_get_string(json_object_object_get(ethLinkObj, "LowerLayers")), "Cellular.Interface.%hhu", &(iid.idx[0]));
        zcfgFeJsonObjFree(ethLinkObj);
        #endif
...
void zcfgFeDalShowNetworkMAP(struct json_object *Jarray){
...
#ifdef ZYXEL_SUPPORT_HOST_MAC_ADDRESS_OUI
	printf("%-20s %-17s %-30s %-20s %-20s %-16s %-5s\n",
		    "Name", "IP Address", "IPv6 Address", "MAC Address", "MAC Address OUI", "Address Source", "Connection Type");
#else
	...
	for(i=0;i<len;i++){
		...
		if(!strcmp(json_object_get_string(json_object_object_get(obj, "PhysAddress")),""))
			continue;
#ifdef ZYXEL_SUPPORT_HOST_MAC_ADDRESS_OUI
		if(!strcmp(json_object_get_string(json_object_object_get(obj, "X_ZYXEL_PhysAddressOUI")),""))
			continue;
#endif
		...
		printf("%-20s %-17s %-30s %-20s %-20s %-16s %-5s \n",
			...
			json_object_get_string(json_object_object_get(obj, "X_ZYXEL_PhysAddressOUI")),
