...
zcfgRet_t zcfgFeDalCellWanStatusGet(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg){
	...
	bool intfZyNbrEnableAll = false;
	bool intfZyNbrEnable = false;
	struct json_object *intfZyNbrJarray = NULL, *intfZyNbrObj = NULL, *intfZyNbrEnableObj = NULL;
#endif
	...
	if((ret = zcfgFeObjJsonGet(RDM_OID_CELL_INTF, &iid, &intfObj)) == ZCFG_SUCCESS){
		...
		json_object_object_add(paramJobj, "INTF_SiteID", JSON_OBJ_COPY(json_object_object_get(intfObj, "X_ZYXEL_SiteID")));
	...
	if((ret = zcfgFeObjJsonGet(RDM_OID_CELL_INTF_ZY_NR_NSA, &iid, &intfZyNrNsaObj)) == ZCFG_SUCCESS){
		...
		json_object_object_add(paramJobj, "NSA_DL_BW", JSON_OBJ_COPY(json_object_object_get(intfZyNrNsaObj, "DL_BW")));
...
#if 0
	IID_INIT(iid);
	if((ret = zcfgFeObjJsonGet(RDM_OID_CELL_ZY_NBR, &iid, &intfZyNbrEnableObj)) == ZCFG_SUCCESS){
		intfZyNbrEnableAll = json_object_get_boolean(json_object_object_get(intfZyNbrEnableObj, "Enable"));
		zcfgFeJsonObjFree(intfZyNbrEnableObj);
	}

	intfZyNbrJarray = json_object_new_array();
	if(intfZyNbrEnableAll){
		IID_INIT(iid);
...
void zcfgFeDalShowCellWanStatus(struct json_object *Jarray){
	...
	if(-1 == json_object_get_int(json_object_object_get(obj, "INTF_SiteID")))
		printf("%-30s %-10s \n","eNodeB ID:", "N/A");
	else
		printf("%-30s %-10s \n","eNodeB ID:", json_object_get_string(json_object_object_get(obj, "INTF_SiteID")));

	if (strncmp(json_object_get_string(json_object_object_get(obj, "INTF_Current_Access_Technology")), "NR5G", strlen("NR5G"))==0) {	
		switch(json_object_get_int(json_object_object_get(obj, "INTF_Downlink_Bandwidth"))){
			...
			case 11: 
				...
				break;				
			case 12: 
				printf("%-30s %-10s \n","DL Bandwidth (MHz):", "100");
				break;
			case 13: 
				printf("%-30s %-10s \n","DL Bandwidth (MHz):", "200");
				break;
			case 14: 
	...
	if (!strncmp(json_object_get_string(json_object_object_get(obj, "INTF_Current_Access_Technology")), "NR-NSA",strlen("NR-NSA"))) {
		printf("\nNR-NSA Information:\n");
		if(-1 == json_object_get_int(json_object_object_get(obj, "NSA_MNC")))
			printf("%-30s %-10s \n","NNC:", "N/A");
		else
			printf("%-30s %-10s \n","NNC:",json_object_get_string(json_object_object_get(obj, "NSA_MNC")));
		if(-1 == json_object_get_int(json_object_object_get(obj, "NSA_MCC")))
			printf("%-30s %-10s \n","MCC:", "N/A");
		else 
			printf("%-30s %-10s \n","MCC:",json_object_get_string(json_object_object_get(obj, "NSA_MCC")));
		if(-1 == json_object_get_int(json_object_object_get(obj, "NSA_PCI")))
			printf("%-30s %-10s \n","Physical Cell ID:", "N/A");
		else 
			printf("%-30s %-10s \n","Physical Cell ID:",json_object_get_string(json_object_object_get(obj, "NSA_PCI")));
		if(-1 == json_object_get_int(json_object_object_get(obj, "NSA_Band")))
			printf("%-30s %-10s \n","Band:", "N/A");	
		else {
#ifdef TELIA_CUSTOMIZATION
			if (!strcmp(json_object_get_string(json_object_object_get(obj, "NSA_Band")), "NR_N1"))
				printf("%-30s %-10s \n","Band:", "NR_N1 (NR 2100)");
			else if (!strcmp(json_object_get_string(json_object_object_get(obj, "NSA_Band")), "NR_N3"))
				printf("%-30s %-10s \n","Band:", "NR_N3 (NR 1800)");
			else if (!strcmp(json_object_get_string(json_object_object_get(obj, "NSA_Band")), "NR_N5"))
				printf("%-30s %-10s \n","Band:", "NR_N5 (NR 850)");
			else if (!strcmp(json_object_get_string(json_object_object_get(obj, "NSA_Band")), "NR_N7"))
				printf("%-30s %-10s \n","Band:", "NR_N7 (NR 2600)");
			else if (!strcmp(json_object_get_string(json_object_object_get(obj, "NSA_Band")), "NR_N8"))
				printf("%-30s %-10s \n","Band:", "NR_N8 (NR 900)");
			else if (!strcmp(json_object_get_string(json_object_object_get(obj, "NSA_Band")), "NR_N20"))
				printf("%-30s %-10s \n","Band:", "NR_N20 (NR 800)");
			else if (!strcmp(json_object_get_string(json_object_object_get(obj, "NSA_Band")), "NR_N28"))
				printf("%-30s %-10s \n","Band:", "NR_N28 (NR 700 APT)");
			else if (!strcmp(json_object_get_string(json_object_object_get(obj, "NSA_Band")), "NR_N38"))
				printf("%-30s %-10s \n","Band:", "NR_N38 (NR TD 2600)");
			else if (!strcmp(json_object_get_string(json_object_object_get(obj, "NSA_Band")), "NR_N40"))
				printf("%-30s %-10s \n","Band:", "NR_N40 (NR TD 2300)");
			else if (!strcmp(json_object_get_string(json_object_object_get(obj, "NSA_Band")), "NR_N41"))
				printf("%-30s %-10s \n","Band:", "NR_N41 (NR TD 2600+)");
			else if (!strcmp(json_object_get_string(json_object_object_get(obj, "NSA_Band")), "NR_N77"))
				printf("%-30s %-10s \n","Band:", "NR_N77 (NR TD 3700)");
			else if (!strcmp(json_object_get_string(json_object_object_get(obj, "NSA_Band")), "NR_N78"))
				printf("%-30s %-10s \n","Band:", "NR_N78 (NR TD 3500)");			
#else			
			
			printf("%-30s %-10s \n","Band:",json_object_get_string(json_object_object_get(obj, "NSA_Band")));
#endif			
		}	
		if(-1 == json_object_get_int(json_object_object_get(obj, "NSA_RFCN")))
			printf("%-30s %-10s \n","RFCN:", "N/A");		
		else	
			printf("%-30s %-10s \n","RFCN:",json_object_get_string(json_object_object_get(obj, "NSA_RFCN")));		
		if(-140  == json_object_get_int(json_object_object_get(obj, "NSA_RSRP")))
			printf("%-30s %-10s \n","RSRP:", "N/A");		
		else			
			printf("%-30s %-10s \n","RSRP:",json_object_get_string(json_object_object_get(obj, "NSA_RSRP")));
		if(-240 == json_object_get_int(json_object_object_get(obj, "NSA_RSRQ")))
			printf("%-30s %-10s \n","RSRQ:", "N/A");		
		else			
			printf("%-30s %-10s \n","RSRQ:",json_object_get_string(json_object_object_get(obj, "NSA_RSRQ")));
		if(-20 == json_object_get_int(json_object_object_get(obj, "NSA_SINR")))
			printf("%-30s %-10s \n","SINR:", "N/A");		
		else			
			printf("%-30s %-10s \n","SINR:",json_object_get_string(json_object_object_get(obj, "NSA_SINR")));

                switch(json_object_get_int(json_object_object_get(obj, "NSA_DL_BW"))){
                        case 0:
                                printf("%-30s %-10s \n","DL Bandwidth (MHz):", "5");
                                break;
                        case 1:
	...
	if(!strncmp(json_object_get_string(json_object_object_get(obj, "INTF_Current_Access_Technology")),"LTE-A",5) || !strncmp(json_object_get_string(json_object_object_get(obj, "INTF_Current_Access_Technology")),"NR-NSA",6)){
	//Show SCC information if SCC is enabled
		...
		while((intfZySccObj = json_object_array_get_idx(json_object_object_get(obj, "SCC_Info"), idxOfScc)) != NULL){
			...
			switch(json_object_get_int(json_object_object_get(intfZySccObj, "DL_BW"))){
				case 0:
					printf("%-30s %-10s \n","DL Bandwidth (MHz):", "1.4");
					break;
				case 1:
			...
			ul_configured = json_object_get_int(json_object_object_get(intfZySccObj, "UL_CONFIGURED"));

			printf("%-30s %-10s \n", "UL Configured:", json_object_get_string(json_object_object_get(intfZySccObj, "UL_CONFIGURED")));
			if (ul_configured) {
				printf("%-30s %-10s \n","UL RFCN:", json_object_get_string(json_object_object_get(intfZySccObj, "UL_RFCN")));

				switch(json_object_get_int(json_object_object_get(intfZySccObj, "UL_BW"))){
					case 0:
						printf("%-30s %-10s \n","UL Bandwidth (MHz):", "1.4");
						break;
					case 1:
						printf("%-30s %-10s \n","UL Bandwidth (MHz):", "3");
						break;
