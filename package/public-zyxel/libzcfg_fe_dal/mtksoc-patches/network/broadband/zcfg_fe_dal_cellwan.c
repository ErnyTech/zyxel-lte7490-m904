...
#endif
#ifdef ZYXEL_PROXY_ARP
{"ProxyARP_Enable", dalType_boolean, 0, 0, NULL},
#endif
...
{"Refresh_Interval", dalType_int, 1, 60, NULL, NULL, 0},
{ NULL,	0,	0,	0,	NULL}
...
zcfgRet_t setCellObj(struct json_object *Jobj){
	...
	refresh_interval = json_object_get_int(json_object_object_get(Jobj, "Refresh_Interval"));
	...
	if((ret = zcfgFeObjJsonGetWithoutUpdate(RDM_OID_CELL, &cellIid, &cellObj)) == ZCFG_SUCCESS){
		...
		if(json_object_object_get(Jobj, "Refresh_Interval"))
			json_object_object_add(cellObj, "X_ZYXEL_Refresh_Interval", json_object_new_int(refresh_interval));
...
zcfgRet_t zcfgFeDalCellWanGet(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg){
	...
	if((ret = zcfgFeObjJsonGet(RDM_OID_CELL, &cellIid, &cellObj)) == ZCFG_SUCCESS){
		...
		json_object_object_add(paramJobj, "Refresh_Interval", JSON_OBJ_COPY(json_object_object_get(cellObj, "X_ZYXEL_Refresh_Interval")));
		zcfgFeJsonObjFree(cellObj);
	}
...
void zcfgFeDalShowCellWan(struct json_object *Jarray){
	...
	printf("%-30s %-10s \n", "Refresh Interval:",Jgets(obj, "Refresh_Interval"));
#ifndef ZYXEL_WEB_GUI_CELLWAN_SUPPORT_MULTI_APN
