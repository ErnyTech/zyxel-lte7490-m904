...
#ifdef RILCMD_SUPPORT_DUAL_SIM
{"USIM_SLOT", dalType_int, 0, 2, NULL, NULL, 0},
{"USIM_DETACH_SWITCH", dalType_boolean, 0, 0, NULL},
#endif
{ NULL,	0,	0,	0,	NULL}
...
{"Nickname", dalType_string, 0, 0, NULL},
{ NULL, 0,      0,      0,      NULL}
...
zcfgRet_t zcfgFeDalCellWanSimEdit(struct json_object *Jobj, char *replyMsg){
	...
	#ifdef RILCMD_SUPPORT_DUAL_SIM
	slot = json_object_get_int(json_object_object_get(Jobj, "USIM_SLOT"));
	detachSwitch = json_object_get_boolean(json_object_object_get(Jobj, "USIM_DETACH_SWITCH"));
	#endif
	...
	if((ret = zcfgFeObjJsonGetWithoutUpdate(RDM_OID_CELL_INTF_USIM, &intfUsimIid, &intfUsimObj)) == ZCFG_SUCCESS){
		#ifdef RILCMD_SUPPORT_DUAL_SIM
		if(json_object_object_get(Jobj, "USIM_SLOT")) {
			json_object_object_add(intfUsimObj, "X_ZYXEL_Slot", json_object_new_int(slot));
			json_object_object_add(intfUsimObj, "X_ZYXEL_STATUS", json_object_new_string("Setting"));
		}
		if (json_object_object_get(Jobj, "USIM_DETACH_SWITCH"))
			json_object_object_add(intfUsimObj, "X_ZYXEL_Detach_Switch", json_object_new_boolean(detachSwitch));
		#endif
...
zcfgRet_t zcfgFeDalCellWanSimGet(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg){
	...
	if((ret =  zcfgFeObjJsonGet(RDM_OID_CELL_INTF_USIM, &intfUsimIid, &intfUsimObj)) == ZCFG_SUCCESS){
		...
		#ifdef RILCMD_SUPPORT_DUAL_SIM
		json_object_object_add(paramJobj, "USIM_SLOT", JSON_OBJ_COPY(json_object_object_get(intfUsimObj, "X_ZYXEL_Slot")));
		json_object_object_add(paramJobj, "USIM_DETACH_SWITCH", JSON_OBJ_COPY(json_object_object_get(intfUsimObj, "X_ZYXEL_Detach_Switch")));
		json_object_object_add(paramJobj, "X_ZYXEL_CURRENT_SLOT", JSON_OBJ_COPY(json_object_object_get(intfUsimObj, "X_ZYXEL_CURRENT_SLOT")));
		json_object_object_add(paramJobj, "X_ZYXEL_STATUS", JSON_OBJ_COPY(json_object_object_get(intfUsimObj, "X_ZYXEL_STATUS")));
		#endif
...
void zcfgFeDalShowCellWanSim(struct json_object *Jarray){
	...
	#ifdef RILCMD_SUPPORT_DUAL_SIM
	printf("%-30s %-10s \n","SLOT:", Jgets(obj, "USIM_SLOT"));
	printf("%-30s %-10s \n","Current SLOT:", Jgets(obj, "X_ZYXEL_CURRENT_SLOT"));
	if(json_object_get_boolean(json_object_object_get(obj, "USIM_DETACH_SWITCH")))
		printf("%-30s %-10s\n","Detach Switch:", "Enable");
	else
		printf("%-30s %-10s\n","Detach Switch:", "Disable");
	printf("%-30s %-10s \n","Setting Status:", Jgets(obj, "X_ZYXEL_STATUS"));
	#endif
...
zcfgRet_t zcfgFeDalCellWanESimEdit(struct json_object *Jobj, char *replyMsg){
	...
	if((ret = zcfgFeObjJsonGetWithoutUpdate(ZyEsimRdmOid, &ZyEsimIid, &ZyEsimObj)) == ZCFG_SUCCESS){
		...
		json_object_object_add(ZyEsimObj, "Status", json_object_new_string("Setting"));
	} else {
...
zcfgRet_t zcfgFeDalCellWanESimProfileEdit(struct json_object *Jobj, char *replyMsg){
	...
	if((ret = zcfgFeObjJsonSetWithoutApply(ZyEsimRdmOid, &ZyEsimIid, ZyEsimObj, NULL)) != ZCFG_SUCCESS){
		zcfgFeJsonObjFree(ZyEsimObj);
		printf("zcfgFeObjJsonSet RDM_OID_CELL_ZY_ESIM fail.\n");
		return ret;
	}
	...
	nickname = json_object_get_string(json_object_object_get(Jobj, "Nickname"));
	...
	if((ret = zcfgFeObjJsonGetWithoutUpdate(ZyEsimProfileRdmOid, &ZyEsimProfileIid, &ZyEsimProfileObj)) == ZCFG_SUCCESS){
		if(json_object_object_get(Jobj, "Profile_Enable")){
			json_object_object_add(ZyEsimProfileObj, "Enable", json_object_new_boolean(profileEnable));
                ...
                if(json_object_object_get(Jobj, "Nickname")){
                        json_object_object_add(ZyEsimProfileObj, "Nickname", json_object_new_string(nickname));
                }
...
zcfgRet_t zcfgFeDalCellWanESimProfileDelete(struct json_object *Jobj, char *replyMsg){
	...
	ZyEsimProfileIid.idx[0] = index;
		...
		json_object_object_add(ZyEsimProfileObj, "Type", json_object_new_string("delete"));
...
zcfgRet_t zcfgFeDalCellWanESimGet(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg){
	...
	if((ret = zcfgFeObjJsonGetWithoutUpdate(ZyEsimRdmOid, &ZyEsimIid, &ZyEsimObj)) == ZCFG_SUCCESS){
		...
		json_object_object_add(paramJobj, "Status", JSON_OBJ_COPY(json_object_object_get(ZyEsimObj, "Status")));
