...
dal_param_t MAIL_NOTIFICATION_param[]={
	...
	{"SMTPSecurity",						dalType_string,	0,	0,	NULL,	"ssl|starttls|none",	dal_Add},
...
zcfgRet_t zcfgFeDalMailNotificationEdit(struct json_object *Jobj, char *replyMsg){
	...
	ret = convertIndextoIid(index, &emailIid, RDM_OID_MAIL_SERVICE, NULL, NULL, replyMsg);
	if(ret != ZCFG_SUCCESS){
		return ret;
	}

	if((ret = zcfgFeObjJsonGet(RDM_OID_MAIL_SERVICE, &emailIid, &mailNotificationJobj)) != ZCFG_SUCCESS) {
	...
	if((ret = zcfgFeObjJsonSet(RDM_OID_MAIL_SERVICE, &emailIid, mailNotificationJobj, NULL)) != ZCFG_SUCCESS)
	{
...
zcfgRet_t zcfgFeDalMailNotificationGet(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg){
	...
	while(zcfgFeObjJsonGetNextWithoutUpdate(RDM_OID_MAIL_SERVICE, &emailIid, &emailObj) == ZCFG_SUCCESS){
		...
		json_object_object_add(paramJobj, "AuthPass", JSON_OBJ_COPY(json_object_object_get(emailObj, "AuthPass")));
		...
		if(json_object_object_get(Jobj, "Index")){
			indexObjToGet = Jgeti(Jobj, "Index");
			if(indexObjToGet == emailIid.idx[0]){
...
zcfgRet_t zcfgFeDalMailNotification(const char *method, struct json_object *Jobj, struct json_object *Jarray, char *replyMsg)
{
	...
	if(!strcmp(method, "POST")) {
		...
		ret = zcfgFeDalMailNotificationEdit(Jobj, NULL);
	}
