...
void zcfgFeDalShowMgmtSrv(struct json_object *Jarray){
	...
	int i, len = 0;
	struct json_object *obj, *remoSrvJarray, *remodata;
	int Index, Port;
...
#ifdef TELIA_CUSTOMIZATION
	remodata = json_object_array_get_idx(Jarray, 0);
	remoSrvJarray = json_object_object_get(remodata, "RemoSrv");
	obj = json_object_array_get_idx(remoSrvJarray, 0);
	len = json_object_array_length(remoSrvJarray);
#else
	...
	for(i=0;i<len;i++){
	#ifdef TELIA_CUSTOMIZATION
		obj = json_object_array_get_idx(remoSrvJarray, i);
	#else
