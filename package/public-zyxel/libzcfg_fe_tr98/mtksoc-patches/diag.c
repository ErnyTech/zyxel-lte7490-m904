...
/*
 *   TR98 Object Name : InternetGatewayDevice.IPLayerCapacityMetrics
 *
 *   Related object in TR181:
 *   Device.IP.Diagnostics.IPLayerCapacityMetrics
 */
zcfgRet_t ipLayerCapMetricsObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	...
	IID_INIT(objIid);
	if((ret = feObjJsonGet(RDM_OID_IP_DIAG_IP_LAYER_CAP_METRICS, &objIid, &sshJobj, updateFlag)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	*tr98Jobj = json_object_new_object();
	...
	while(paramList->name != NULL){
		...
		if(paramValue != NULL){
			//printf("%s : name %s, value %s\n", __FUNCTION__, paramList->name, json_object_get_string(paramValue));
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
		}
 ...
 *   Device.IP.Diagnostics.IPLayerCapacityMetrics
 */
zcfgRet_t ipLayerCapMetricsObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *ipPingObj = NULL;
	...
	IID_INIT(objIid);
	if((ret = zcfgFeObjJsonGet(RDM_OID_IP_DIAG_IP_LAYER_CAP_METRICS, &objIid, &ipPingObj)) != ZCFG_SUCCESS)
		return ret;
	...
	if(multiJobj){
		...
		tmpObj = ipPingObj;
		ipPingObj = NULL;
		ipPingObj = zcfgFeJsonMultiObjAppend(RDM_OID_IP_DIAG_IP_LAYER_CAP_METRICS, &objIid, multiJobj, tmpObj);
		json_object_put(tmpObj);
		zcfgFeJsonMultiObjAddParam(multiJobj, RDM_OID_IP_DIAG_IP_LAYER_CAP_METRICS, &objIid, "partialset", json_object_new_boolean(true));
	}
	...
	while(paramList->name != NULL) {
		...
		if(!strcmp(paramList->name, "DiagnosticsState")) {
			...
			if(spvParamStr && strcmp(spvParamStr, "Requested") && strcmp(spvParamStr, "Canceled"))
				return ZCFG_INVALID_PARAM_VALUE;
	...
	if(multiJobj){
		...
		//if((ret = zcfgFeObjJsonSet(RDM_OID_IP_DIAG_IP_LAYER_CAP_METRICS, &objIid, ipPingObj, NULL)) != ZCFG_SUCCESS ) {
		const char *ipping;
		ipping = json_object_to_json_string(ipPingObj);
		if((ret = zcfgFeObjWrite(RDM_OID_IP_DIAG_IP_LAYER_CAP_METRICS, &objIid, ipping, REQUEST_SET_OBJ | ZCFG_PARTIAL_OBJECT_SET, NULL)) != ZCFG_SUCCESS ) {
			json_object_put(ipPingObj);
			return ret;
		}
		json_object_put(ipPingObj);
...
/*
 *   TR98 Object Name : InternetGatewayDevice.IPLayerCapacityMetrics.IncrementalResult.{i}
 *
 *   Related object in TR181:
 *   Device.IP.Diagnostics.IPLayerCapacityMetrics.IncrementalResult.{i}
 */
zcfgRet_t ipLayerIncreResObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *ipLayerIncreResObj = NULL;
	struct json_object *paramValue = NULL;
	tr98Parameter_t *paramList = NULL;

	printf("%s : Enter\n", __FUNCTION__);

	const char *strP = strstr(tr98FullPathName, "IncrementalResult");
	if(!strP)
		return ZCFG_NO_SUCH_OBJECT;

	IID_INIT(objIid);
	objIid.level = 1;
	if(sscanf(strP, "IncrementalResult.%hhu", &objIid.idx[0]) != 1)
		return ZCFG_NO_SUCH_OBJECT;

	if((ret = feObjJsonGet(RDM_OID_IP_DIAG_IP_LAYER_INCRE_RES, &objIid, &ipLayerIncreResObj, updateFlag)) != ZCFG_SUCCESS){
		ztrdbg(ZTR_DL_DEBUG, "%s: retrieve RDM_OID_IP_DIAG_IP_LAYER_INCRE_RES fail!\n", __FUNCTION__);
		printf("%s: retrieve RDM_OID_IP_DIAG_IP_LAYER_INCRE_RES fail!\n", __FUNCTION__);
		return ret;
	}
	...
	*tr98Jobj = json_object_new_object();
	paramList = tr98Obj[handler].parameter;
	while(paramList->name != NULL) {
		paramValue = json_object_object_get(ipLayerIncreResObj, paramList->name);
		if(!paramValue) {
			//printf("Can't find parameter %s in TR181\n", paramList->name);
			paramList++;
			continue;
	...
	json_object_put(ipLayerIncreResObj);

	return ZCFG_SUCCESS;
}

/*
 *   TR98 Object Name : InternetGatewayDevice.IPLayerCapacityMetrics.ModalResult.{i}
 *
 *   Related object in TR181:
 *   Device.IP.Diagnostics.IPLayerCapacityMetrics.ModalResult.{i}
 */
zcfgRet_t ipLayerModalResObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *ipLayerIncreResObj = NULL;
	...
	const char *strP = strstr(tr98FullPathName, "ModalResult");
	if(!strP)
		return ZCFG_NO_SUCH_OBJECT;

	IID_INIT(objIid);
	objIid.level = 1;
	if(sscanf(strP, "ModalResult.%hhu", &objIid.idx[0]) != 1)
		return ZCFG_NO_SUCH_OBJECT;

	if((ret = feObjJsonGet(RDM_OID_IP_DIAG_IP_LAYER_MODAL, &objIid, &ipLayerIncreResObj, updateFlag)) != ZCFG_SUCCESS){
		ztrdbg(ZTR_DL_DEBUG, "%s: retrieve RDM_OID_IP_DIAG_IP_LAYER_MODAL fail!\n", __FUNCTION__);
		//printf("%s : retrieve RDM_OID_IP_DIAG_IP_LAYER_MODAL fail\n", __FUNCTION__);
		return ret;
	}
