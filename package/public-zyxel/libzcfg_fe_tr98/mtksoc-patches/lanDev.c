...
#ifdef LANIPALIAS
	bool ipIntfv4AddrInstGet = false;
#endif
...
#ifdef LANIPALIAS
	while((ret = feObjJsonSubNext(RDM_OID_IP_IFACE_V4_ADDR, &ipIntfIid, &ipIntfv4AddrIid, &ipIntfv4AddrJobj, updateFlag)) == ZCFG_SUCCESS){
		if(!strcmp("true", json_object_to_json_string(json_object_object_get(ipIntfv4AddrJobj, "X_ZYXEL_Alias")))){
			json_object_put(ipIntfv4AddrJobj);
			continue;
		}
		else{
			//The interface should only has one real IP and other aliases.
			ipIntfv4AddrInstGet = true;
			break;
		}
	}

	if(!ipIntfv4AddrInstGet){
...
#ifdef LANIPALIAS
	while((ret = zcfgFeSubInObjJsonGetNext(RDM_OID_IP_IFACE_V4_ADDR, &ipIntfIid, &ipIntfv4AddrIid, &ipIntfv4AddrJobj)) == ZCFG_SUCCESS){
		if(!strcmp("true", json_object_to_json_string(json_object_object_get(ipIntfv4AddrJobj, "X_ZYXEL_Alias")))){
			json_object_put(ipIntfv4AddrJobj);
			continue;
		}
