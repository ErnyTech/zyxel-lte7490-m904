...
#define LEVEL_NAME_UD "UserDefined" //for Firewall Level
#endif
...
/*From DefaultGateway get interface name*/
	...
	else if(strstr(tr181Obj, "Cellular") != NULL) {
		rdm_CellIntf_t *cellIntf = NULL;
		rdm_CellIntfSt_t *cellIntfSt = NULL;

		json_object_object_add(*tr98Jobj, "WANAccessType", json_object_new_string("Cellular"));
		sscanf(tr181Obj, "Cellular.Interface.%hhu", &objIid.idx[0]);

		if(feObjStructGet(RDM_OID_CELL_INTF, &objIid, (void **)&cellIntf, updateFlag) == ZCFG_SUCCESS) {
			if(!strcmp(cellIntf->Status, "Up") || !strcmp(cellIntf->Status, "Down")) {
				json_object_object_add(*tr98Jobj, "PhysicalLinkStatus", json_object_new_string(cellIntf->Status));
			}

			zcfgFeObjStructFree(cellIntf);
		}

		if(feObjStructGet(RDM_OID_CELL_INTF_ST, &objIid, (void **)&cellIntfSt, updateFlag) == ZCFG_SUCCESS) {
			json_object_object_add(*tr98Jobj, "TotalBytesSent", json_object_new_int(cellIntfSt->BytesSent));
			json_object_object_add(*tr98Jobj, "TotalBytesReceived", json_object_new_int(cellIntfSt->BytesReceived));
			json_object_object_add(*tr98Jobj, "TotalPacketsSent", json_object_new_int(cellIntfSt->PacketsSent));
			json_object_object_add(*tr98Jobj, "TotalPacketsReceived", json_object_new_int(cellIntfSt->PacketsReceived));
			zcfgFeObjStructFree(cellIntfSt);
		}
	}
#endif
...
zcfgRet_t WANIpConnObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	...
	bool runSpvValid;
	#ifdef TELIA_CUSTOMIZATION //for Firewall Level
	objIndex_t levelIid;
...
//#endif
	...
	if(rtJobj)
		...
		if(levelObj){
			tmpObj = levelObj;
			levelObj = NULL;
			levelObj = zcfgFeJsonMultiObjAppend(RDM_OID_FIREWALL_LEVEL, &levelIid, multiJobj, tmpObj);			
			json_object_put(tmpObj);
			zcfgFeJsonMultiObjAddParam(multiJobj, RDM_OID_FIREWALL_LEVEL, &levelIid, "apply", json_object_new_boolean(true));
		}
		#endif
		...
		if(paramValue != NULL) {
			...
			if(!strcmp(paramList->name, "ConnectionType")) {
				...
				*/
				...
				bool natEnabledValue = json_object_get_boolean(paramValue);
				
				if(1 == natEnabledValue){
				...
				if(levelObj){
					...
					zcfgFeJsonMultiObjAddParam(multiJobj, RDM_OID_FIREWALL_LEVEL, &levelIid, "X_ZYXEL_DefaultConfig", json_object_new_string(firewall_level));
				}
				#endif
	...
	if(multiJobj){
		...
		if(levelObj){
			if((ret = zcfgFeObjJsonSet(RDM_OID_FIREWALL_LEVEL, &levelIid, levelObj, NULL)) != ZCFG_SUCCESS ) {
				zcfgLog(ZCFG_LOG_ERR, "%s : Set Device.Firewall.Level Fail\n", __FUNCTION__);
				printf("%s : Set Device.Firewall.Level Fail\n", __FUNCTION__);
			}
			else {
				zcfgLog(ZCFG_LOG_DEBUG, "%s : Set Device.Firewall.Level Success\n", __FUNCTION__);
				printf("%s : Set Device.Firewall.Level Success\n", __FUNCTION__);
			}
...
zcfgRet_t WANIpConnObjAdd(char *tr98FullPathName, int *idx)
{
	...
	*ptr = '\0';
	...
	else if(strstr(wanIntfObj, "Cellular") != NULL) { // Cellular WAN
		wanIpConnType = WAN_IPCONN_EINTELVLIP;
		wanLinkName = wanIntfObj;
	}
	else {
...
/*  TR98 Object Name : InternetGatewayDevice.WANDevice.i.X_ZYXEL_WANCellAccessPoint
 *
 *  Related object in TR181:
 *  Device.Cellular.AccessPoint.i
 */
zcfgRet_t WANCellAccessPointObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	zcfgFeSetParmAttr_t *setParmAttr = NULL;
	uint32_t seqnum = 0;
	objIndex_t tr98ObjIid, tr181ObjIid;
	...
	struct json_object *jobj = NULL;
	char ipIfaceName[TR98_MAX_OBJ_NAME_LENGTH] = {0};
	struct json_object *cellIntfJobj = NULL;
	zcfgRet_t ret;
	struct json_object *paramValue = NULL;
	tr98Parameter_t *paramList = NULL;
	...
	if(zcfgFe98To181ObjMappingGet(seqnum, &tr98ObjIid, &oid, &tr181ObjIid) != ZCFG_SUCCESS)	{
		printf("%s: no %s mapping\n", __FUNCTION__, tr98FullPathName);
		return ZCFG_NO_SUCH_OBJECT;
	}

	if(oid != RDM_OID_CELL_ACCESS_POINT)
		return ZCFG_NO_SUCH_OBJECT;

	if((ret = feObjJsonGet(oid, &tr181ObjIid, &cellIntfJobj, updateFlag)) != ZCFG_SUCCESS)
		return ret;

	printf("Cellular Interface %s\n", json_object_to_json_string(cellIntfJobj));

	/*fill up tr98 WANEthernetInterfaceConfig object*/
	*tr98Jobj = json_object_new_object();
	paramList = tr98Obj[handler].parameter;
	while(paramList->name != NULL) {
		paramValue = json_object_object_get(cellIntfJobj, paramList->name);
		if(paramValue != NULL) {
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
...
/*  TR98 Object Name : InternetGatewayDevice.WANDevice.i.X_ZYXEL_WANCellAccessPoint
    ...
    Device.Cellular.AccessPoint.i
 */
zcfgRet_t WANCellAccessPointObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	zcfgFeSetParmAttr_t *setParmAttr = NULL;
	uint32_t seqnum = 0;
	objIndex_t tr98ObjIid, tr181ObjIid;
	...
	if(oid != RDM_OID_CELL_ACCESS_POINT)
		...
		if((ret = zcfgFeObjJsonGet(RDM_OID_CELL_ACCESS_POINT, &tr181ObjIid, &cellIntfJobj)) != ZCFG_SUCCESS)
			return ret;

		if(multiJobj){
			tmpObj = cellIntfJobj;
			cellIntfJobj = NULL;
			cellIntfJobj = zcfgFeJsonMultiObjAppend(RDM_OID_CELL_ACCESS_POINT, &tr181ObjIid, multiJobj, tmpObj);
		}
		...
		while(paramList->name != NULL) {
			...
			paramValue = json_object_object_get(tr98Jobj, paramList->name);
			if(paramValue != NULL) {
				tr181ParamValue = json_object_object_get(cellIntfJobj, paramList->name);
				if(tr181ParamValue != NULL) {
					json_object_object_add(cellIntfJobj, paramList->name, JSON_OBJ_COPY(paramValue));
					paramList++;
					continue;
				}
			}
		...
		if(multiJobj){
			...
			if((ret = zcfgFeObjJsonSet(RDM_OID_CELL_ACCESS_POINT, &tr181ObjIid, cellIntfJobj, NULL)) != ZCFG_SUCCESS ) {
				json_object_put(cellIntfJobj);
				return ret;
			}
			json_object_put(cellIntfJobj);
...
/*  TR98 Object Name : InternetGatewayDevice.WANDevice.i.X_ZYXEL_WANCellInterfaceConfig
 *
 *  Related object in TR181:
 *  Device.Cellular.Interface.i
 */
zcfgRet_t WANCellIfaceCfgObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	zcfgFeSetParmAttr_t *setParmAttr = NULL;
	uint32_t seqnum = 0;
	objIndex_t tr98ObjIid, tr181ObjIid;
...
/*  TR98 Object Name : InternetGatewayDevice.WANDevice.i.X_ZYXEL_WANCellInterfaceConfig
    ...
    Device.Cellular.Interface.i
 */
zcfgRet_t WANCellIfaceCfgObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	zcfgRet_t ret;
	char tr98TmpName[TR98_MAX_OBJ_NAME_LENGTH] = {0};
	char cellIntfName[32] = {0};
	char *ptr = NULL;
	...
	/*find tr181 mapping object*/
	strcpy(tr98TmpName, tr98FullPathName);
	ptr = strstr(tr98TmpName, ".X_ZYXEL_WANCellInterfaceConfig");
	*ptr = '\0';

	/* tr98TmpName will be InternetGatewayDevice.WANDevice.i */
	if((ret = zcfgFe98To181MappingNameGet(tr98TmpName, cellIntfName)) == ZCFG_SUCCESS) {
		if(strstr(cellIntfName, "Cellular") == NULL) {
			printf("%s : Not an Cellular Interface\n", __FUNCTION__);
			return ZCFG_NO_SUCH_OBJECT;
		}

		IID_INIT(cellIntfIid);
		sscanf(cellIntfName, "Cellular.Interface.%hhu", &cellIntfIid.idx[0]);
		cellIntfIid.level = 1;

		if((ret = zcfgFeObjJsonGet(RDM_OID_CELL_INTF, &cellIntfIid, &cellIntfJobj)) != ZCFG_SUCCESS)
			return ret;

		if(multiJobj){
			tmpObj = cellIntfJobj;
			cellIntfJobj = NULL;
			cellIntfJobj = zcfgFeJsonMultiObjAppend(RDM_OID_CELL_INTF, &cellIntfIid, multiJobj, tmpObj);
		}
		...
		if(multiJobj){
			...
			if((ret = zcfgFeObjJsonSet(RDM_OID_CELL_INTF, &cellIntfIid, cellIntfJobj, NULL)) != ZCFG_SUCCESS ) {
				json_object_put(cellIntfJobj);
				return ret;
			}
			json_object_put(cellIntfJobj);
...
/*  TR98 Object Name : InternetGatewayDevice.WANDevice.i.X_ZYXEL_WANCellInterfaceConfig.Stats

    Related object in TR181:
    Device.Cellular.Interface.i.Stats.
 */
zcfgRet_t WANCellIfaceCfgStObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	zcfgFeSetParmAttr_t *setParmAttr = NULL;
	uint32_t seqnum = 0;
	objIndex_t tr98ObjIid, tr181ObjIid;
