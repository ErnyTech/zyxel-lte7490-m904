...
//===========================================================================================================
/*
 *   TR98 Object Name : InternetGatewayDevice.DeviceInfo.TemperatureStatus. 
 *
 *   Related object in TR181: Device.DeviceInfo.TemperatureStatus.
 *
 */
zcfgRet_t devTempStGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = feObjJsonGet(RDM_OID_TEMP_STAT, &objIid, &sshJobj, updateFlag)) != ZCFG_SUCCESS){
		return ret;
	}
...
zcfgRet_t devTempStNotify(char *tr98ObjName, char *tr181ParamName, struct json_object *tr181ParamVal, int handler, struct json_object **tr98NotifyInfo)
{
	bool found = false;
	char tr98Notify[256] = "";
	tr98Parameter_t *paramList = NULL;
...
int devTempStAttrGet(char *tr98FullPathName, int handler, char *paramName)
{
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
	uint32_t  sshOid = 0;
...
zcfgRet_t devTempStAttrSet(char *tr98FullPathName, int handler, char *paramName, int newNotify, struct json_object *multiAttrJobj)
{
	zcfgRet_t ret = ZCFG_SUCCESS;
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
...
/*
 *   TR98 Object Name : InternetGatewayDevice.DeviceInfo.TemperatureStatus.TemperatureSensor.i
 *
 *   Related object in TR181: Device.DeviceInfo.TemperatureStatus.TemperatureSensor.i
 *
 */
zcfgRet_t devTempStSensorGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *sshJobj = NULL;
	...
	IID_INIT(objIid);
	objIid.level = 1;
	sscanf(tr98FullPathName, "InternetGatewayDevice.DeviceInfo.TemperatureStatus.TemperatureSensor.%hhu", &objIid.idx[0]);

	if((ret = feObjJsonGet(RDM_OID_TEMP_STAT_TEMP_SENSOR, &objIid, &sshJobj, updateFlag)) != ZCFG_SUCCESS){
		return ret;
	}
...
zcfgRet_t devTempStSensorSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *sshJobj = NULL;
	...
	if((ret = zcfgFeObjJsonGet(RDM_OID_TEMP_STAT_TEMP_SENSOR, &objIid, &sshJobj)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	if(multiJobj){
		tmpObj = sshJobj;
		sshJobj = NULL;
		sshJobj = zcfgFeJsonMultiObjAppend(RDM_OID_TEMP_STAT_TEMP_SENSOR, &objIid, multiJobj, tmpObj);
	}
	...
	if(multiJobj){
		...
		if((ret = zcfgFeObjJsonSet(RDM_OID_TEMP_STAT_TEMP_SENSOR, &objIid, sshJobj, NULL)) != ZCFG_SUCCESS ){
			json_object_put(sshJobj);
			return ret;
		}
		json_object_put(sshJobj);
...
zcfgRet_t devTempStSensorAdd(char *tr98FullPathName, int *idx)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	...
	IID_INIT(objIid);
	ret = zcfgFeObjStructAdd(RDM_OID_TEMP_STAT_TEMP_SENSOR, &objIid, NULL);
	*idx = objIid.idx[0];

	return ret;
}

zcfgRet_t devTempStSensorDel(char *tr98FullPathName)
{
	objIndex_t objIid;
	...
	return zcfgFeObjStructDel(RDM_OID_TEMP_STAT_TEMP_SENSOR, &objIid, NULL);;
}

zcfgRet_t devTempStSensorNotify(char *tr98ObjName, char *tr181ParamName, struct json_object *tr181ParamVal, int handler, struct json_object **tr98NotifyInfo)
{
	bool found = false;
	char tr98Notify[256] = "";
	tr98Parameter_t *paramList = NULL;
...
int devTempStSensorAttrGet(char *tr98FullPathName, int handler, char *paramName)
{
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
	uint32_t  sshOid = 0;
...
zcfgRet_t devTempStSensorAttrSet(char *tr98FullPathName, int handler, char *paramName, int newNotify, struct json_object *multiAttrJobj)
{
	zcfgRet_t ret = ZCFG_SUCCESS;
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
...
//===========================================================================================================
//===========================================================================================================
#endif //#ifdef ZYXEL_CELLWAN_WITH_LTE_MODULE_TEMPERATURE