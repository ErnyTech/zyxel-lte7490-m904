...
int sethandler(int argc, char **argv)
{
	...
	for(_index=0;_index<argc;++_index)
	{
		...
		// John, replace the above code due to build failure.
		strncpy(ori_message_backup,argv[_index],sizeof(ori_message_backup));
...
void show_help()
{
	...
	ZTR69_COMMONLOG(ZTR69_COMMON_LOG_LEVEL_OUTPUT,"   del                del datamodel object in {PATH}; del {PATH}\n");
