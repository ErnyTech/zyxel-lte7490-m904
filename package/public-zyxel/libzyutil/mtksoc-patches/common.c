...
// hashedValue is used when the data model's password is hashed not encrypted
int zyUtilAuthUserAccount(const char *username, const char *password, const char *hashedValue)
{
	...
	char syscmd[512];
	int fd, r, ret=-1;
	char *hashedValueOfInputPw = NULL;
	char salt[512] = {0};
	int i = 0, signCount = 0, hashedValueLen = 0;

	if (username == NULL || password == NULL) return -3;

	//For the case that first time to generate password by MAC/SN, the password hasn't been written to flash.
	//So it is plain text in memory, not hashed value.
	if((hashedValue != NULL) && (strncmp(hashedValue, "$6$", 3) != 0)){
		return 1;
	}

	if(zyGetAccountInfoFromFile(username, "/etc/passwd") != NULL){
		//The account exists in /etc/passwd, then use login tool to verify if it is legal.
		...
		//No account in /etc/password, then use input hashedValue to verify if it is legal.
		if(hashedValue == NULL){
			ret = -1;
			goto out;
		}
		...
		if(strncmp(hashedValue, "$6$", 3) == 0){

			i = 0;
			signCount = 0;
			hashedValueLen = strlen(hashedValue);
			//Get the salt of the hashed value
			while(hashedValue[i] != '\0' && i < hashedValueLen){
				if(hashedValue[i] == '$'){
					signCount++;
				}
				if(signCount == 3){
				...
				salt[i] = hashedValue[i];
				i++;
			}

			
			hashedValueOfInputPw = (char*)sha512Hash(password, salt);
			if(hashedValueOfInputPw != NULL){
				if(strcmp(hashedValue, hashedValueOfInputPw) == 0){
					printf("%s:%d, hash matched!.\n", __func__, __LINE__);
					ret = 0;
					goto out;
				}
				else{
					printf("%s:%d, hash not matched!.\n", __func__, __LINE__);
					ret = -1;
					goto out;
				}
			}
			else{
				printf("%s:%d, Unknown error, hashedValueOfInputPw is NULL!.\n", __func__, __LINE__);
				ret = -1;
				goto out;
			}
		}
		else{
			printf("%s:%d The hashedValue(%s) is not SHA512 hashed.\n", __func__, __LINE__, hashedValue);
			ret = -1;
			goto out;
		}
	}
...
out:
	//remember to free the return value of sha512HashWithSalt()
	if(hashedValueOfInputPw != NULL){
		free(hashedValueOfInputPw);
		hashedValueOfInputPw = NULL;
	}
	return ret;
}
#else
...
char *zyGetAccountInfoFromFile(char *username, char *filePath){
    ...
    while(fgets(accountInfo, sizeof(accountInfo), fp) != NULL){
        if(strncmp(username, accountInfo, strlen(username)) == 0){
			...
			if(accountInfo[strlen(accountInfo) - 1] == '\n'){
				accountInfo[strlen(accountInfo) - 1] = '\0';
			}
			account_info_ret = strdup(accountInfo);
            fclose(fp);
            return account_info_ret;
        }
    }

    //printf("%s: cannot find account '%s' in '%s'!\n", __FUNCTION__, username, filePath);
    fclose(fp);
    return NULL;
}
...
/* avoid using system() for command injection issue
*/
int
execute_command (char *command, char *arg1, char *arg2, char *arg3, char *arg4, char *arg5, char *arg6, char *arg7, char *arg8, char *arg9,char *arg10 )
{
  int ret;
  pid_t pid;
  int status;
  ...
  */
  ...
  if (pid < 0)
  ...
  else if (pid == 0)
    {
      ...
      ret = execlp (command, command, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10, NULL);
  ...
  else
    {
	  ...
	  printf("execute_command(%s) done, return status %d\n\n",command, status);
	  return status;
    }
...
#if defined(ZYXEL_RANDOM_ENCRYPTION_SEED_IN_MRD)
//dig2str convert dig to str with heximal format
int dig2str(unsigned char* dig, int dig_len, char* str, int str_len) {
	unsigned char* dp = dig;
	char* cp = str;
	int 	ii;

	for (ii = 0; ii < dig_len; ii++, cp += 2, dp++)
	{
		if(((ii + 1) > dig_len) || (((ii + 1)*2) > str_len)){
			//over the string size, avoiding crash
			break;
		}
...
void mergeRandomSeedKey(char *key){
	struct mrd mrd;
	unsigned char MD5_Output[MD5_RANDOM_SEED_INPUT_SIZE] = { 0 };
	char MD5_Input[MD5_RANDOM_SEED_INPUT_SIZE] = { 0 };
	int i = 0, keyLen = 0;
	char *input = NULL;
	char *prefix = MRD_RANDOM_SEED_PREFIX;
	bool validSeed = true;

	//printf("%s(%d) The input key is %s.\n", __func__, __LINE__, key);
	zyutil_dbg_printf("%s(%d) INFO: The input key is %s.\n", __func__, __LINE__, key);
	keyLen = strlen(key);
	if(keyLen <= 0 || keyLen > MD5_RANDOM_SEED_INPUT_SIZE){
		zyutil_dbg_printf("%s(%d) ERROR : key(%d) %s is invalid.\n", __func__, __LINE__, keyLen, key);
		return;
	}

	if (zyUtilGetMrdInfo(&mrd) != 0) {
		zyutil_dbg_printf("%s(%d) ERROR : zyUtilGetMrdInfo failed.\n", __func__, __LINE__);
		return;
	}

	while(prefix[i] != '\0'){
		if((unsigned char)prefix[i] != mrd.EncryptionSeed[i]){
			validSeed = false;
		}
		i++;
	}
	...
	if(validSeed){
		input = mrd.EncryptionSeed;
		input += strlen(prefix); //remove the prefix
		snprintf(MD5_Input, MD5_RANDOM_SEED_INPUT_SIZE, "%s%s",  input, key);
		//empty the key
		memset(key, 0, MRD_KEY_LENGTH);

		//printf("%s(%d) the MD5_Input is %s.\n", __func__, __LINE__, MD5_Input);

		//get the MD5 hash value with random seed combined original key
		getExpectedContext(Encrypt_OPENSSL_MD5, MD5_Input, MD5_Output);
		//convert the result(16 Bytes) to heximal format(32 Bytes)
		dig2str(MD5_Output, strlen(MD5_Output), key, MRD_KEY_LENGTH);

		if(keyLen >= 0 && keyLen < MRD_KEY_LENGTH){
			//cut it to the original key size(should be 27)
			key[keyLen] = '\0';
		}
		else{
			key[MRD_KEY_LENGTH - 1] = '\0';
		}
	}
	//printf("%s(%d) the key(%d) is %s, original key length is %d.\n", __func__, __LINE__, strlen(key), key, keyLen);
}
#endif
#endif
