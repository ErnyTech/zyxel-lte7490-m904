...
TARGET_CONFIGURE_OPTS = \
  ...
  LD="$(TARGET_CROSS)ld -rpath-link $(STAGING_DIR)/usr/lib" \
  NM="$(TARGET_NM)" \
  CC="$(TARGET_CC) -march=armv7-a -mthumb -mfpu=neon -mfloat-abi=hard -std=gnu89" \
  GCC="$(TARGET_CC) -march=armv7-a -mthumb -mfpu=neon -mfloat-abi=hard" \
  CXX="$(TARGET_CXX) -march=armv7-a -mthumb -mfpu=neon -mfloat-abi=hard" \
...
ifeq ($(CONFIG_TARGET_brcm963xx),y)
	...
	ifeq ($(CONFIG_TARGET_quectel),y)
		ZCFG_PLATFORM=QUECTEL
		USE_MTKSOC_PATCH=zcfg_be,libzcfg_msg,logrotate,clinkc,zcmd,zhttpd,libzcfg_fe_tr98,libzcfg_fe_dal,dalcmd,\
				ztr69cmd,ztr69,zycfgfilter,zupnp,esmd,libzyutil,udhcp
	endif
endif
export ZCFG_PLATFORM
export USE_MTKSOC_PATCH
