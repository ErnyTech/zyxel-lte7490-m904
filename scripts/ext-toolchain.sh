...
probe_cc() {
	if [ -z "$CC" ]; then
		local bin
		local binPATH=""bin" "usr/bin" "usr/local/bin""
		local configFILE="$TOPDIR/.config"

		if [ -e $configFILE ]; then
			local BOARD=`grep CONFIG_TARGET_BOARD $configFILE | cut -d '=' -f 2`
			if [ "$BOARD" == '"quectel"' ]; then
				local FIND_OPT=`grep -w CONFIG_TARGET_quectel $configFILE | cut -d '=' -f 2`

				if [ "$FIND_OPT" == "y" ]; then
					binPATH="bin/arm-oe-linux-gnueabi $binPATH"
				fi
			fi
		fi
