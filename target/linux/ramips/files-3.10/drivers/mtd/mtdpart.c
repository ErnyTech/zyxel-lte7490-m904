...
#define ZYXEL_TRX_HEADER_LENGTH         0x17c
...
static void split_uimage(struct mtd_info *master, struct mtd_part *part)
{
	...
	if (hdr.magic == cpu_to_be32(TRX_MAGIC) || hdr.magic == cpu_to_be32(TRX_MAGIC1) || hdr.magic == cpu_to_be32(TRX_MAGIC2))
		zytrx_offset=ZYXEL_TRX_HEADER_LENGTH;
		//part->offset=part->offset+ZYXEL_TRX_HEADER_LENGTH;
		//part->mtd.size=part->mtd.size-ZYXEL_TRX_HEADER_LENGTH;
		if (mtd_read(master, part->offset+zytrx_offset, sizeof(hdr), &len, (void *) &hdr))
		{
		int i = master->erasesize;
		while (mtd_read(master, part->offset +i+zytrx_offset, sizeof(hdr), &len, (void *) &hdr))
			i+= master->erasesize;
			//len=len+ZYXEL_TRX_HEADER_LENGTH;
		//return;
                }
	...
	if (hdr.kern_size != 0 && hdr.name[0] == 0)
		len = be32_to_cpu(hdr.kern_size) + zytrx_offset;
	else
	len = be32_to_cpu(hdr.size) + 0x40 + zytrx_offset;
