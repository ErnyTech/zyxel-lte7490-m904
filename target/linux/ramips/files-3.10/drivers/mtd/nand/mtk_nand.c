...
static int is_skip_bad_block(struct mtd_info *mtd, int page)
{
	...
	count ++;
//ZYXEL Comm. Corp., James, 20180807: Disable bad block skip mechanism for dual kernel(Start).
	//if ((page << chip->page_shift) >= FILE_SYSTEM_START_ADDRESS)
  /* Limit to UBIFS partition range, it can't use skip bad block mechanism. */
#if 1
  if (((page << chip->page_shift) >= MTD_DATA_PART_OFFSET) && \
    ((page << chip->page_shift) < MTD_ROMD_PART_OFFSET))
	{
		return 0;
	}
	//return 1;
  /* We handled bad block in user space mtd.c, so just return 0 */
  return 0;
#endif
//ZYXEL Comm. Corp., James, 20180807: Disable bad block skip mechanism for dual kernel(END).
}
