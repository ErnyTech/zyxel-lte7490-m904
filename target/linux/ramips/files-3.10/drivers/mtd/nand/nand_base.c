		    ...
		    int allowbbt)
{
	...
	/*Add for handle exceed memory*/
	int page, status, pages_per_block, ret, chipnr, block, mapped_block;
	/*End of handle exceed memory*/
	struct nand_chip *chip = mtd->priv;
	loff_t len;
	...
	while (len) {
		...
		if (nand_block_checkbad(mtd, ((loff_t) page) <<
			...
			pr_warn("%s: attempt to erase a bad block at page 0x%08x\n",
				    __func__, page);
//ZYXEL Comm. Corp., James, 20180807: Checking remapping mechanism(Start).
			/*Add for handle bad block remapping, the result is to find next good block.*/
				block = ((int)instr->addr/mtd->erasesize);
				mapped_block = check_block_remap(mtd, block);
				//mapped_block = block +1;
				pr_warn("%s: (kernel)mapping bad block to block number %d\n",
					    __func__, mapped_block);
				if (mapped_block == -1)
				{
					instr->state = MTD_ERASE_FAILED;
					goto erase_exit;
				}
				len -= (1 << chip->phys_erase_shift);	/* Shift 1 block */
				page = pages_per_block * mapped_block;
				continue;

			/* We already handled bad block, don't return fail. */
			//instr->state = MTD_ERASE_FAILED;
			//goto erase_exit;
//ZYXEL Comm. Corp., James, 20180807: Checking remapping mechanism(END).
		}
