...
void RtmpOsPktNatMagicTag(IN struct sk_buff* pRxPkt)
{
...
#if defined (CONFIG_RA_HW_NAT_NIC_USB)
	skb = __netdev_alloc_skb_ip_align (dev->net, size + FOE_INFO_LEN, flags);
#else
...
static void usbnet_bh (unsigned long param)
{
	...
	struct skb_data		*entry;
#ifdef CONFIG_OPTIMIZED_MEMCPY_IN_GOBINET
	if(skb_queue_len(&dev->done)> 1024) {
		printk("Done queue len=%d\n",skb_queue_len(&dev->done));
	}
#endif
...
#if defined (CONFIG_MULTI_APN_SUPPORT) //in multi-apn QMAP mode, use usbx(usb0) as interface name
                        strcpy (net->name, "usb%d");
#elif defined (CONFIG_ZYXEL)  //change the interface name for Gobinet		
...
#ifndef CONFIG_MULTI_APN_SUPPORT //No need to REG when MultiAPN/QMAP mode is enable 
#if defined(CONFIG_RA_HW_NAT_NIC_USB)
#ifdef CONFIG_RA_HW_NAT_WIFI_NEW_ARCH
	RT_MOD_HNAT_REG(net);
#endif
