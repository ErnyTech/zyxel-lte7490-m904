...
#ifdef CONFIG_RA_HW_NAT_SUPPORT_QOS_8021Q_BY_TC
	u16 vlanPbit; 
	if(FOE_ENTRY_NUM(skb)==0x3fff || FOE_ENTRY_NUM(skb) >= FOE_4TB_SIZ) {
		//some special alert occurred, so entry_num is useless (just skip it)
	} else {
		vlanPbit = (entry->ipv4_hnapt.resv2 >> 13) & 0x7;
		if(vlanPbit >=0 && vlanPbit <=7){
			skb->mark |= (8 - vlanPbit);
		}
	}
#endif
...
#ifdef CONFIG_RA_HW_NAT_SUPPORT_QOS_8021Q_BY_TC
	uint8_t* skbData;
	u16 vlanId = 0, vlanPrio = 10, i = 0;

	skbData = (uint8_t *) skb_mac_header(skb);
	for (i = 0; i < 16; i++){
		//printk(" %02X", skbData[i]);
		if((skbData[i] == 0x81 && skbData[i+1] == 0x00)){
			vlanId = ((skbData[i+2] << 8) | skbData[i+3]) & VLAN_VID_MASK;
			vlanPrio = (((skbData[i+2] << 8) | skbData[i+3]) & 0xE000) >> 13;
			
			if(FOE_ENTRY_NUM(skb)==0x3fff || FOE_ENTRY_NUM(skb) >= FOE_4TB_SIZ) {
				//some special alert occurred, so entry_num is useless (just skip it)
			} else {
				entry->ipv4_hnapt.resv2 = 0;
				entry->ipv4_hnapt.resv2 |= vlanPrio;
				entry->ipv4_hnapt.resv2 = entry->ipv4_hnapt.resv2 << 13;
				entry->ipv4_hnapt.resv2 |= vlanId;			
				//printk("%s(%d) i is %d.\n", __func__, __LINE__, i);
			}
		}
	}
